...
!     ------------------------------------------------------------------
        ...
        if (me%delfun <= 0.) me%delfun = .0001
        me%ct = -abs(me%ct)
        if (me%ct >= 0.) me%ct = -.1
        me%ctmin = abs(me%ctmin)
        if (me%ctmin <= 0.) me%ctmin = .004
        me%ctl = -abs(me%ctl)
        if (me%ctl >= 0.) me%ctl = -0.01
        me%ctlmin = abs(me%ctlmin)
        if (me%ctlmin <= 0.) me%ctlmin = .001
        if (me%theta <= 0.) me%theta = 1.
        if (me%abobj1 <= 0.) me%abobj1 = .1
        if (me%alphax <= 0.) me%alphax = .1
        if (me%fdch <= 0.) me%fdch = .01
        if (me%fdchm <= 0.) me%fdchm = .01
!     ------------------------------------------------------------------
...
60      me%obj1 = me%obj
        ...
        if (me%dabfun < 1.0d-10) me%dabfun = 1.0d-10
        if (me%iprint > 0) then
!     ------------------------------------------------------------------
...
130     me%iter = me%iter + 1
        if (me%abobj1 < .0001) me%abobj1 = .0001
        if (me%abobj1 > .2) me%abobj1 = .2
        if (me%alphax > 1.) me%alphax = 1.
        if (me%alphax < .001) me%alphax = .001
!
...
!     ------------------------------------------------------------------
        if (s(ndv1) < 1.0d-6 .and. me%nvc == 0) go to 450
!     ------------------------------------------------------------------
...
380     alp1 = 1.0d+20
        si = abs(me%obj)
        if (si < .01) si = .01
        if (abs(me%slope) > 1.0d-20) alp1 = me%abobj1*si/me%slope
        alp1 = abs(alp1)
        if (me%nvc > 0) alp1 = 10.*alp1
        if (alp1 < me%alp) me%alp = alp1
!     ------------------------------------------------------------------
...
!     ------------------------------------------------------------------
        ...
        do i = 1, me%ndv
            si = abs(s(i))
            me%xi = abs(x(i))
            if (si >= 1.0d-10 .and. me%xi >= 0.1) then
                alp1 = me%alphax*me%xi/si
                if (alp1 < alp11) alp11 = alp1
            end if
        end do
        if (me%nvc > 0) alp11 = 10.*alp11
        if (alp11 < me%alp) me%alp = alp11
        if (me%alp > 1.0d+20) me%alp = 1.0d+20
        if (me%alp <= 1.0d-20) me%alp = 1.0d-20
        if (me%iprint >= 3) then
            write (6, 9100)
            do i = 1, me%ndv, 6
...
!     ABSOLUTE CHANGE IN OBJECTIVE.
        ...
        if (me%nac == 0 .or. objb > 0.) me%ncobj = 0
...
!     ------------------------------------------------------------------
            if (abs(me%obj1) > 1.0d-10) objd = objd/abs(me%obj1)
